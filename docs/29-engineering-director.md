...
The audit rates `engineering-framework` as high-value but duplicated across `.agents` and `.codex`; current installed files remain untouched. A future Director implementation must first prove canonical path/host precedence, then migrate behavior through benchmark, not assumption. Source evidence: [`skill-audit/reports/03-static-audit.md`](../skill-audit/reports/03-static-audit.md) and [`13-authority-model.md`](../skill-audit/reports/13-authority-model.md).
