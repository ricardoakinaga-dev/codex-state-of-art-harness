...
const elements = {};
let queueRequest = 0;
let retainedIntakeRequest = null;
...
function queueEndpoint(recover = false, requestId = 0) {
  const scenario = recover && currentScenario() === "error" ? "recovered" : currentScenario();
  const params = new URLSearchParams({ scenario });
  if (scenario === "stale-response") params.set("request", String(requestId));
...
function renderStateRow(title, detail, actionLabel, actionHandler, tone = "") {
  ...
  if (actionLabel) {
    ...
    action.type = "button";
    if (tone === "error") action.dataset.recoveryAction = "true";
...
async function loadQueue(recover = false) {
  const recoveryActionFocused =
    document.activeElement?.dataset.recoveryAction === "true" ||
    document.activeElement === elements.summaryRetry;
  const requestId = ++queueRequest;
  setState({ phase: "loading", error: "" });
  try {
    const response = await fetch(queueEndpoint(recover, requestId), { headers: { Accept: "application/json" } });
    ...
    if (recoveryActionFocused) elements.refreshQueue.focus();
  } catch {
    if (requestId !== queueRequest) return;
    setState({ phase: "error", error: "The queue could not be loaded. Try again." });
  }
}
...
async function submitIntake(event) {
  ...
  const serializedDraft = JSON.stringify(result.values);
  if (!retainedIntakeRequest || retainedIntakeRequest.serializedDraft !== serializedDraft) {
    retainedIntakeRequest = Object.freeze({ key: newIdempotencyKey(), serializedDraft });
  }
  let failureMessage = "The intake could not be sent.";
  try {
    const params = new URLSearchParams({ scenario: currentScenario() });
    const response = await fetch(`/api/intakes?${params.toString()}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json", "Idempotency-Key": retainedIntakeRequest.key },
      body: retainedIntakeRequest.serializedDraft,
    });
    const payload = await response.json();
    if (!response.ok || payload.status === "error") {
      if (payload.errors && typeof payload.errors === "object") showFormErrors(payload.errors);
      if (typeof payload.message === "string" && payload.message.length <= 160) failureMessage = payload.message;
      throw new Error("intake-request-failed");
    }
    elements.form.reset();
    showFormErrors({});
    retainedIntakeRequest = null;
    ...
    setState({ submitting: false, feedback: `${failureMessage} You can safely try again.`, feedbackTone: "error" });
  }
}
...
function bind() {
  ...
  render();
  if (currentScenario() !== "stale-response") loadQueue();
}
