  ...
  if (items.length === 0) {
    ...
    renderStateRow("The desk is clear.", "There are no active cases in this view right now.", current.items.length === 0 ? "Refresh queue" : "Show all", () => {
      if (current.items.length === 0) loadQueue();
        ...
        setState({ selectedUrgency: "all" });
      }
    });
    return;
  }
  elements.queueState.textContent = current.selectedUrgency === "all" ? `${items.length} active cases · sorted by arrival` : `${items.length} ${URGENCY_LABELS[current.selectedUrgency].toLowerCase()} cases`;
...
function bind() {
  ...
  elements.urgencyFilter.addEventListener("change", (event) => setState({ selectedUrgency: event.target.value }));
