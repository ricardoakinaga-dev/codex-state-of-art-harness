...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_uses_sqlite_result_code_when_lock_message_is_opaque(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        sentinel = object()

        def coded_lock_then_success() -> Any:
            nonlocal attempts
            attempts += 1
            if attempts == 1:
        ...
        with patch("app.service.time.sleep") as sleep:
            result = service._with_write_retry(coded_lock_then_success, "req-coded-lock")
        service.close()
