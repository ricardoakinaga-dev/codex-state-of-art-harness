...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_uses_sqlite_result_code_for_lock_bursts(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        sentinel = object()

        def coded_lock_burst() -> Any:
            nonlocal attempts
            attempts += 1
            if attempts <= 2:
                error = sqlite3.OperationalError("opaque transient writer contention")
                error.sqlite_errorcode = sqlite3.SQLITE_BUSY | (7 << 8)
                raise error
            return sentinel

        with patch("app.service.time.sleep") as sleep:
            result = service._with_write_retry(coded_lock_burst, "req-coded-lock")
        service.close()
