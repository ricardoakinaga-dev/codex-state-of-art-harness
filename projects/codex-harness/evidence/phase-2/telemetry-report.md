...
telemetry trunca eventos com limitation observável. Duração só aparece quando
há medição factual: `TOOL_RESULT` preserva a duração do provider e
`RUN_COMPLETED` soma as durações das invocations; eventos de controle não
recebem o valor fictício `1`. Um log corrompido não é alterado; a persistência
registra diagnóstico seguro e ainda grava o resultado computado e seu summary.
Writes são atômicos dentro da fronteira.
