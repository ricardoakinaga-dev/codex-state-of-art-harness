...
class PilotTests(unittest.TestCase):
    ...
    def test_idempotency_replay_rejects_mismatched_persisted_appointment(self) -> None:
        created = create_appointment(self.db_path, "actor-1", "idem-mismatch", PAYLOAD)
        self.assertEqual(created.status_code, 201)
        tampered = {**created.body, "data": {"appointment_id": "apt_other", "status": "BOOKED"}}
        with sqlite3.connect(self.db_path) as connection:
            connection.execute(
                "UPDATE idempotency_keys SET response_json = ? WHERE actor_id = ? AND key = ?",
                (json.dumps(tampered), "actor-1", "idem-mismatch"),
            )
            connection.commit()

        replay = create_appointment(self.db_path, "actor-1", "idem-mismatch", PAYLOAD)
        self.assertEqual(replay.status_code, 500)
        self.assertEqual(replay.body["error"]["code"], "INTERNAL_ERROR")
