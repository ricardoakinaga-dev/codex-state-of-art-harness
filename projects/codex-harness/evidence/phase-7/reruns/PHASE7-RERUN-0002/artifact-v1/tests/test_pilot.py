...
class PilotTests(unittest.TestCase):
    ...
    def test_corrupt_saved_idempotency_response_fails_closed(self) -> None:
        cases = (
            ...
            ("unsupported-status", "response_status", 202),
            (
                "mismatched-appointment",
                "response_json",
                json.dumps(
                    {
                        "success": True,
                        "data": {"appointment_id": "apt_tampered", "status": "BOOKED"},
                        "error": None,
                        "meta": {"request_id": "req_saved"},
                    }
                ),
            ),
        )
        for index, (name, column, tampered_value) in enumerate(cases):
            with self.subTest(name=name):
                key = f"idem-corrupt-{index}"
                payload = dict(PAYLOAD, starts_at=f"2026-09-01T1{index}:00:00Z")
                created = create_appointment(self.db_path, "actor-1", key, payload)
                self.assertEqual(created.status_code, 201)
                with sqlite3.connect(self.db_path) as connection:
                    if column == "response_json":
                        connection.execute(
                            "UPDATE idempotency_keys SET response_json = ? "
                            "WHERE actor_id = ? AND key = ?",
                            (tampered_value, "actor-1", key),
                        )
                    else:
                        connection.execute(
                ...
                result = create_appointment(self.db_path, "actor-1", key, payload)
