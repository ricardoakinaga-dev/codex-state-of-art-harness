...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_discards_locked_connection_and_partial_transaction(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        first_connection: sqlite3.Connection | None = None

        def transient_lock_after_partial_write() -> Any:
            nonlocal attempts, first_connection
            attempts += 1
            ...
            if attempts == 1:
                ...
                connection.execute(
                    ...
                    ("provider-partial", "Partial attempt", 1),
                )
                raise sqlite3.OperationalError("database is locked")
            self.assertIsNot(connection, first_connection)
            row = connection.execute(
                "SELECT id FROM providers WHERE id = ?", ("provider-partial",)
            ).fetchone()
        ...
        try:
            with patch("app.service.time.sleep"):
                result = service._with_write_retry(
                    transient_lock_after_partial_write, "req-clean-retry"
                )
