...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_discards_connection_with_an_open_transaction(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        failed_connection: sqlite3.Connection | None = None

        def lock_after_transaction_starts() -> Any:
            nonlocal attempts, failed_connection
            attempts += 1
            ...
            if attempts == 1:
                ...
                raise sqlite3.OperationalError("database is locked")
            self.assertIsNot(connection, failed_connection)
        ...
        try:
            with patch("app.service.time.sleep", return_value=None):
                result = service._with_write_retry(
                    lock_after_transaction_starts,
                    "req-open-transaction",
                )
