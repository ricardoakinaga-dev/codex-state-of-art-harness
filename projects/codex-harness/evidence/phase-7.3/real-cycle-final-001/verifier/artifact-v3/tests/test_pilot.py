...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_recognizes_extended_sqlite_busy_code(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        busy = sqlite3.OperationalError("opaque sqlite failure")
        ...
        def operation() -> Any:
            ...
            return create_appointment(
                self.db_path, "actor-1", "idem-extended-busy", PAYLOAD
            )

        try:
            with patch("app.service.time.sleep") as sleep:
                result = service._with_write_retry(operation, "req_extended_busy")
        finally:
            service.close()
