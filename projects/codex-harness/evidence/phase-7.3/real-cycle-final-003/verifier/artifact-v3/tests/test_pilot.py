...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_jitters_backoff_within_explicit_bounds(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        sleeps: list[float] = []

        def transient_lock_burst() -> object:
            nonlocal attempts
            attempts += 1
            if attempts <= 2:
                raise sqlite3.OperationalError("database is locked")
        ...
        with (
            patch("app.service.random.uniform", side_effect=(0.5, 1.5)) as jitter,
            patch("app.service.time.sleep", side_effect=sleeps.append),
        ):
            service._with_write_retry(transient_lock_burst, "req-jitter")
        service.close()

        self.assertEqual(attempts, 3)
        self.assertEqual(sleeps, [0.01, 0.06])
        self.assertEqual(
            [call.args for call in jitter.call_args_list],
            [(0.5, 1.5), (0.5, 1.5)],
        )
    ...
    def test_cli_main_handles_interrupt_and_no_demo_data(self) -> None:
        ...
        with (
            ...
            result = cli.main(
                ["--db", str(self.db_path), "--log", str(self.root / "cli.jsonl")]
            )
    ...
    def test_concurrent_different_keys_preserve_unique_slot(self) -> None:
        ...
        def worker(index: int) -> None:
            result = create_appointment(
                self.db_path, "actor-1", f"idem-slot-{index}", PAYLOAD
            )
            with lock:
                results.append(result)
