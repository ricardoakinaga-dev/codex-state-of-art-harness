...
def _migrate_in_process(db_path: str, start_event: Any, results: Any) -> None:
...
class PilotTests(unittest.TestCase):
    ...
    def test_migrations_are_safe_when_processes_initialize_together(self) -> None:
        migration_path = self.root / "multiprocess.sqlite3"
        ...
        processes = [
            context.Process(
                ...
                args=(str(migration_path), start_event, results),
            )
            for _ in range(10)
        ]
        try:
            ...
            self.assertEqual(outcomes, ["OK"] * len(processes))
    ...
    def test_write_retry_tolerates_a_transient_lock_burst(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0
        sentinel = object()

        def transient_lock_burst() -> Any:
            nonlocal attempts
            attempts += 1
            if attempts <= 3:
                raise sqlite3.OperationalError("database is locked")
        ...
        result = service._with_write_retry(transient_lock_burst, "req-transient-lock")
        service.close()
    ...
    def test_corrupt_saved_idempotency_response_fails_closed(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as connection:
            connection.execute(
                "UPDATE idempotency_keys SET response_json = ? WHERE actor_id = ? AND key = ?",
    ...
    def test_write_retry_jitters_lock_retries_and_preserves_busy_contract(self) -> None:
        service = AppointmentService(self.db_path)
        attempts = 0

        def transient_lock() -> Any:
            nonlocal attempts
            attempts += 1
            if attempts <= 2:
                locked = sqlite3.OperationalError("transient write contention")
                locked.sqlite_errorcode = sqlite3.SQLITE_BUSY
                raise locked
            return create_appointment(
                self.db_path, "actor-1", "idem-after-lock-burst", PAYLOAD
            )

        try:
            with (
                patch("app.service.random.uniform", side_effect=(0.015, 0.045)) as jitter,
                patch("app.service.time.sleep") as sleep,
            ):
                result = service._with_write_retry(transient_lock, "req_lock_burst")

            self.assertEqual(result.status_code, 201)
            self.assertEqual(attempts, 3)
            self.assertEqual(
                jitter.call_args_list,
                [
                    unittest.mock.call(0.01, 0.03),
                    unittest.mock.call(0.02, 0.06),
                ],
            )
            self.assertEqual(
                sleep.call_args_list,
                [unittest.mock.call(0.015), unittest.mock.call(0.045)],
            )
        finally:
            service.close()

        always_locked = AppointmentService(self.db_path)
        try:
            with (
                patch("app.service.random.uniform", return_value=0),
                patch("app.service.time.sleep"),
            ):
                exhausted = always_locked._with_write_retry(
            ...
            self.assertEqual(exhausted.body["error"]["code"], "CONFLICT")
            self.assertEqual(exhausted.body["meta"]["request_id"], "req_busy")
        finally:
            always_locked.close()
    ...
    def test_http_rejects_duplicate_nonfinite_and_oversized_bodies(self) -> None:
        ...
        try:
            ...
            nonfinite, nonfinite_status, _ = self._request(
                ...
                "POST",
                "/api/v1/appointments",
                b'{"client_id":"client-1","patient_id":"patient-1",'
                b'"provider_id":"provider-1","starts_at":"2026-09-01T11:00:00Z",'
