...
class PilotTests(unittest.TestCase):
    ...
    def test_write_retry_tolerates_structured_transient_lock_bursts(self) -> None:
        service = AppointmentService(self.db_path)
        sentinel = object()
        try:
            for lock_code in (sqlite3.SQLITE_BUSY, sqlite3.SQLITE_LOCKED):
                with self.subTest(lock_code=lock_code):
                    attempts = 0
                    ...
                    def transient_lock_burst() -> Any:
                        ...
                        attempts += 1
                        if attempts <= 3:
                            error = sqlite3.OperationalError("localized contention message")
                            error.sqlite_errorcode = lock_code
                            raise error
                        return sentinel

                    with patch("app.service.time.sleep"):
                        result = service._with_write_retry(
                            transient_lock_burst, "req-transient-lock"
                        )
